    ...
    <Container maxWidth="sm">
      <Box sx={{ mt: 4, mb: 2 }}>
        <Typography color={'primary'} variant="h6">
          {isCreatingCompra ? "Lista de Compras Activa" : "Lista de Compras"}
        </Typography>
      </Box>
      ...
      {!isCreatingCompra && (
        <>
          ...
          <Typography variant="h6" sx={{ mt: 4, mb: 2 }}>Historial de Compras</Typography>
          <List sx={{ mt: 2 }}>
        ...
        <>
          ...
          <List sx={{ mt: 4 }}>
            {shoppingList.map((item, index) => (
              <ListItem key={index} divider sx={{ color: 'black', boxShadow: 2 }}>
                <ListItemText
                  primary={item.name}
                  secondary={
                    <Box component="span" sx={{ display: 'flex', justifyContent: 'space-between' }}>
